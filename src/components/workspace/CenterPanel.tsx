...
export default function CenterPanel({ onMobileBack }: { onMobileBack?: () => void }) {
  ...
  const isFeatureView = !['editor', 'codebase', 'preview'].includes(centerView);
  const showMobileBack = isMobile && isFeatureView && onMobileBack;

  return (
    <main className="workspace-center">
      {showMobileBack ? (
        <div className="center-mobile-bar">
          <button type="button" className="center-mobile-back" onClick={onMobileBack}>
            <ArrowLeft size={18} />
            <span>Tools</span>
          </button>
        </div>
