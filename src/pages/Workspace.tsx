...
import MobileToolsHub from '../components/workspace/MobileToolsHub';
...
export default function Workspace() {
  ...
  const [showToolsHub, setShowToolsHub] = useState(true);
  ...
  useEffect(() => {
    ...
    if (mobilePanel === 'tools' && showToolsHub) return;
    if (mobilePanel === 'files') return;
  }, [isMobile, mobilePanel, setCenterView, showToolsHub]);

  function handleMobilePanelChange(panel: MobilePanel) {
    if (panel === 'tools') setShowToolsHub(true);
  ...
  const workspaceClass = [
    ...
    isMobile && mobilePanel === 'tools' && !showToolsHub ? 'panel-feature' : '',
  ].filter(Boolean).join(' ');
  ...
  return (
    <div className={workspaceClass}>
      ...
      ) : isMobile && mobilePanel === 'tools' && showToolsHub ? (
        <div className="mobile-tools-fullscreen">
          <MobileToolsHub onSelect={() => setShowToolsHub(false)} />
        </div>
      ) : (
        <>
          <LeftSidebar />
          <CenterPanel onMobileBack={() => { setShowToolsHub(true); setMobilePanel('tools'); }} />
          <RightSidebar />
        </>
      )}
      <IntegrationsBar />
      ...
      {isMobile && (
        <MobileWorkspaceNav active={mobilePanel} onChange={handleMobilePanelChange} />
      )}
