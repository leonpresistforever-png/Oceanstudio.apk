...
A signed ARM64 debug/testing APK, version 1.1.3 (code 5), has been built from efc6567ae2021f5b689fc7d684125763a840bb9e. The committed alias is `releases/OceanStudio-latest-debug.apk`. The versioned GitHub release is `v1.1.3`.
...
The live APT pool is unchanged: 1,036 distinct names. The existing pool includes Termux-derived files and lacks complete source provenance. The independent source builder has four pinned recipes. This session rebuilt and retained cmark and kilo; subsequent source downloads were blocked. The attached candidate ZIP is not a live APT update and Android installation/execution tests are pending. Expansion to 2,200 packages and a fully upstream-source-built bootstrap remain unfinished.
