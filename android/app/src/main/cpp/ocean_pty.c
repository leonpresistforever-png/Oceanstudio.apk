...
typedef struct { _Atomic int master; pid_t pid; _Atomic int exit_status; _Atomic int closed; _Atomic int wait_started; int log_fd; } ocean_pty;
...
JNIEXPORT void JNICALL Java_studio_ocean_app_terminal_NativePty_destroy(JNIEnv*env,jclass type,jlong handle){(void)env;(void)type;ocean_pty*p=(ocean_pty*)(intptr_t)handle;if(!p)return;if(!atomic_exchange(&p->closed,1)){int fd=atomic_exchange(&p->master,-1);if(fd>=0)close(fd);}CRUMB(p->log_fd,"[F11] native session destroyed after reader+waiter completion\n");if(p->log_fd>=0)close(p->log_fd);free(p);}
