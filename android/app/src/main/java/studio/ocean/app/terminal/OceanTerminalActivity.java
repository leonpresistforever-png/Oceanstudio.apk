...
public final class OceanTerminalActivity extends AppCompatActivity implements TerminalSession.Listener {
    ...
    private final ServiceConnection connection=new ServiceConnection(){public void onServiceConnected(ComponentName n,IBinder binder){bound=true;OceanTerminalRuntimeService service=((OceanTerminalRuntimeService.LocalBinder)binder).service();session=service.firstRunning();if(session==null)try{session=service.createSession(24,80);}catch(Exception error){status.setText(getString(R.string.terminal_start_failed,error.getMessage()));return;}session.addListener(OceanTerminalActivity.this);LocalProcessDiagnostics.Snapshot process=session.diagnostics(new OceanPaths(OceanTerminalActivity.this).prefix().getAbsolutePath());status.setText((OceanRuntimeState.isInstalled(OceanTerminalActivity.this)?getString(R.string.terminal_runtime_ready):getString(R.string.terminal_recovery_mode))+" · "+process.summary());}public void onServiceDisconnected(ComponentName n){bound=false;session=null;}};
