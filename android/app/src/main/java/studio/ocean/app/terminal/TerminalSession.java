...
public final class TerminalSession {
    ...
    private void maybeFinalize(String caller){if(!readerDone.get()||!waiterDone.get())return;if(!finalizeClaimed.compareAndSet(false,true)){TerminalDiagnosticBundle.log(context,"session-state.log","DOUBLE_FINALIZE_ATTEMPT caller="+caller);return;}synchronized(this){state=State.CLOSING;}TerminalDiagnosticBundle.log(context,"session-state.log","[F04/F10] finalizer claimed caller="+caller);nativeLifetime.writeLock().lock();try{closeMasterOnce("finalizer");nativeDestroyed=true;NativePty.destroy(handle);TerminalDiagnosticBundle.log(context,"session-state.log","[F11] native session destroyed");}finally{nativeLifetime.writeLock().unlock();}synchronized(this){state=State.EXITED;}TerminalDiagnosticBundle.log(context,"session-state.log","[F12] state EXITED code="+exitCode);TerminalDiagnosticBundle.completeAttempt(context,"normal exit code="+exitCode);if(exitDelivered.compareAndSet(false,true))for(Listener listener:listeners)listener.onExit(exitCode);completionListener.onCompleted(this);}
