...
public final class TerminalDiagnosticBundle {
 ...
 public static void beginAttempt(Context c){init(c);synchronized(LOCK){File metadata=new File(root,"attempt.properties");if(metadata.isFile()){Properties previous=load(metadata);File attempts=new File(root,"attempts");attempts.mkdirs();String id=previous.getProperty("id",new SimpleDateFormat("yyyyMMdd-HHmmss-SSS",Locale.US).format(new Date()));File archive=new File(attempts,id);archive.mkdirs();for(String n:LOGS){File f=new File(root,n);if(f.isFile()&&!n.startsWith("previous-process"))f.renameTo(new File(archive,n));}File[] old=attempts.listFiles();if(old!=null&&old.length>8){Arrays.sort(old,Comparator.comparing(File::getName));for(int i=0;i<old.length-8;i++)deleteTree(old[i]);}}Properties next=new Properties();next.setProperty("id",new SimpleDateFormat("yyyyMMdd-HHmmss-SSS",Locale.US).format(new Date()));next.setProperty("active","true");next.setProperty("build",BuildConfig.OCEAN_BUILD_COMMIT);next.setProperty("lastStage","ACTIVITY_CREATED");next.setProperty("test","PRODUCTION TERMINAL");storeAtomic(metadata,next);}log(c,"startup.log","[ATTEMPT] begin");}
 ...
 public static boolean hasStartupCrash(Context c){init(c);Properties p=load(new File(root,"attempt.properties"));if("true".equals(p.getProperty("active")))return true;String e=read("previous-process-exit.log");return e.contains("REASON_CRASH")||e.contains("REASON_SIGNALED")||e.contains("REASON_ANR");}
