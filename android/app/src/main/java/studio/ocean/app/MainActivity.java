...
public class MainActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        if (getSharedPreferences(PREFS,MODE_PRIVATE).contains(TOKEN)) showMain(); else showAuth();
    }
    ...
    private void setAuthMode(AuthMode mode) {
        authMode=mode; TextView title=findViewById(R.id.auth_title), subtitle=findViewById(R.id.auth_subtitle), toggle=findViewById(R.id.auth_switch), forgot=findViewById(R.id.forgot_password), divider=findViewById(R.id.auth_divider); Button primary=findViewById(R.id.auth_primary); View password=findViewById(R.id.auth_password), confirm=findViewById(R.id.auth_confirm), social=findViewById(R.id.social_buttons);
        ...
        password.setVisibility(reset?View.GONE:View.VISIBLE); confirm.setVisibility(signup?View.VISIBLE:View.GONE); forgot.setVisibility(mode==AuthMode.SIGN_IN?View.VISIBLE:View.GONE); social.setVisibility(reset?View.GONE:View.VISIBLE); divider.setVisibility(reset?View.GONE:View.VISIBLE); findViewById(R.id.auth_status).setVisibility(View.GONE);
    ...
    private void submitAuth() {
        ...
        if (!authClient.configured()) { showAuthStatus(getString(R.string.auth_not_configured),false); return; }
        ...
        setAuthBusy(true);
        AuthClient.Callback callback=result -> runOnUiThread(() -> { setAuthBusy(false); if (!result.success) { showAuthStatus(result.message==null?getString(R.string.request_failed):result.message,false); return; } if (authMode==AuthMode.FORGOT) { showAuthStatus(getString(R.string.reset_sent),true); return; } getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(TOKEN,result.token).putString(EMAIL,result.email).apply(); showMain(); });
    ...
    private void showMain() {
        ...
        findViewById(R.id.sign_out).setOnClickListener(v -> { getSharedPreferences(PREFS,MODE_PRIVATE).edit().clear().apply(); showAuth(); });
