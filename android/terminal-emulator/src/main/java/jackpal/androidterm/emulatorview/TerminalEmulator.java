...
class TerminalEmulator {
    ...
    private void selectGraphicRendition() {
        ...
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                i += 2;
            } else if (code == 38 && i+4 <= mArgIndex && mArgs[i+1] == 2) { // foreground truecolor
                mForeColor = trueColor(mArgs[i+2], mArgs[i+3], mArgs[i+4]);
                i += 4;
                ...
                i += 2;
            } else if (code == 48 && i+4 <= mArgIndex && mArgs[i+1] == 2) { // background truecolor
                mBackColor = trueColor(mArgs[i+2], mArgs[i+3], mArgs[i+4]);
                i += 4;
    ...
    private int trueColor(int red, int green, int blue) {
        red = Math.max(0, Math.min(255, red));
        green = Math.max(0, Math.min(255, green));
        blue = Math.max(0, Math.min(255, blue));
        return BaseTextRenderer.registerTrueColor(0xff000000 | red << 16 | green << 8 | blue);
    }
