...
## Local bootstrap-bearing APK build

When hosted distribution runners are unavailable, hydrate the minimal
bootstrap dependency closure from the signed public Ocean repository and build
the APK locally without rebuilding the complete package pool:

```bash
ANDROID_HOME=/path/to/android-sdk \
JAVA_HOME=/path/to/jdk-17 \
./scripts/build-bootstrap-apk-local.sh
```

The hydrator verifies `InRelease`, the pinned repository fingerprint, every
downloaded package SHA-256 and the complete dependency closure. The wrapper
runs unit tests, builds both app and instrumentation APKs, compares the embedded
bootstrap byte-for-byte with the verified input and checks the APK signature.
