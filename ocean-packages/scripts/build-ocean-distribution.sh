...
EXPECTED_REPOSITORY_FINGERPRINT=$(tr -d '[:space:]' < "$ROOT/ocean-packages/keys/ocean-development-repository.fingerprint")
ACTUAL_REPOSITORY_FINGERPRINT=$(gpg --batch --with-colons --fingerprint "$OCEAN_REPO_SIGNING_KEY" | awk -F: '$1=="fpr"{print $10;exit}')
if [[ -z "$ACTUAL_REPOSITORY_FINGERPRINT" || "$ACTUAL_REPOSITORY_FINGERPRINT" != "$EXPECTED_REPOSITORY_FINGERPRINT" ]]; then
  echo "Repository signing key does not match the public key pinned by OceanStudio." >&2
  echo "expected=$EXPECTED_REPOSITORY_FINGERPRINT actual=${ACTUAL_REPOSITORY_FINGERPRINT:-missing}" >&2
  exit 1
fi
...
# Bundle the signed minimal acceptance repository for offline repair, while the
# separately signed public repository supplies the complete on-demand pool.
...
printf 'deb [signed-by=%s/etc/apt/keyrings/ocean.gpg] %s stable main\n' "$OCEAN_PREFIX" "$OCEAN_REPOSITORY_URL" > "$OUT/bootstrap/root$OCEAN_PREFIX/etc/apt/sources.list.d/ocean-online.list"
