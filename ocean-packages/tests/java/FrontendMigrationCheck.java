...
public final class FrontendMigrationCheck {
 ...
 public static void main(String[] args) throws Exception {
  legacy=Files.readAllBytes(Path.of(args[0]));backup=args.length > 2 ? args[2] : "ocean-pkg-1.1.0.backup";replacement=Files.readAllBytes(Path.of(args[1]));
  Path p=root();check(migrate(p));check(Arrays.equals(replacement,Files.readAllBytes(p.resolve("bin/pkg"))));check(Arrays.equals(legacy,Files.readAllBytes(p.resolve("var/lib/dpkg/" + backup)))) ;check(Files.isExecutable(p.resolve("bin/pkg")));check(!migrate(p));
