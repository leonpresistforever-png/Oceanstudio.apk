...
## Repository transport remediation

The Android distribution configuration now uses the anonymously reachable raw
GitHub HTTPS endpoint instead of the unavailable Pages endpoint. The pinned
public key is aligned with the repository's current signing fingerprint,
`09D45DD2CDC37BD4F9BC2C458EC15431CA5542E2`. Distribution assembly fails when
the CI signing key does not match that pinned fingerprint, and a network check
verifies `InRelease`, its signature, and the compressed package index.

This fixes repository discovery and authentication. It does **not** promote the
1,046 indexed records to source-provenance-verified or device-executed status;
the package-level findings and rebuild requirements above remain applicable.
